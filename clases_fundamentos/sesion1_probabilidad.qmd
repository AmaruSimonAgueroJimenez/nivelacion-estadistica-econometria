...
La probabilidad de un evento es la proporción de veces que ocurriría si pudiéramos repetir el experimento una y otra vez en igualdad de condiciones.
:::
:::

## Laboratorio: frecuencia relativa y probabilidad

Mueva los controles y observe cómo la frecuencia relativa acumulada se estabiliza en torno a la probabilidad verdadera (ley de los grandes números).

::: {.content-visible when-format="html"}

```{=html}
<div style="display:flex; gap:1.5em; align-items:center; flex-wrap:wrap; font-size:0.62em; margin-bottom:0.3em;">
  <label>Lanzamientos n: <input type="range" id="lab1-n" min="10" max="5000" step="10" value="500" style="width:170px; vertical-align:middle;"> <span id="lab1-n-val"></span></label>
  <label>p verdadera: <input type="range" id="lab1-p" min="0.1" max="0.9" step="0.01" value="0.42" style="width:170px; vertical-align:middle;"> <span id="lab1-p-val"></span></label>
  <button id="lab1-re" style="font-size:1em; padding:0.15em 0.7em;">Re-simular</button>
</div>
...
(function() {
  function dibujar() {
    ...
    var p = parseFloat(document.getElementById('lab1-p').value);
    document.getElementById('lab1-n-val').textContent = n;
    document.getElementById('lab1-p-val').textContent = p.toFixed(2);
    var xs = [], ys = [], acum = 0;
    for (var i = 1; i <= n; i++) {
      if (Math.random() < p) { acum++; }
      xs.push(i); ys.push(acum / i);
    }
    var frecFinal = ys[n - 1];
    Plotly.react('lab1-plot', [
      {x: xs, y: ys, mode: 'lines', line: {color: '#2E86C1', width: 2}, hoverinfo: 'x+y'},
      {x: [1, n], y: [p, p], mode: 'lines', line: {color: '#E74C3C', width: 2, dash: 'dash'}, hoverinfo: 'skip'}
    ], {margin: {t: 10, r: 20, b: 40, l: 55},
        xaxis: {title: 'numero de lanzamientos', range: [0, n]},
        yaxis: {title: 'frecuencia relativa acumulada', range: [0, 1]},
        showlegend: false},
       {displayModeBar: false});
    document.getElementById('lab1-lectura').textContent =
      'Con n = ' + n + ': frecuencia relativa final = ' + frecFinal.toFixed(3) +
      ' vs p = ' + p.toFixed(2) + ' (distancia ' + Math.abs(frecFinal - p).toFixed(3) +
      '). Suba n y mire como la linea azul se pega a la roja.';
  }
  function init() {
    if (typeof Plotly === 'undefined') { setTimeout(init, 200); return; }
    ['lab1-n', 'lab1-p'].forEach(function(id) {
      document.getElementById(id).addEventListener('input', dibujar);
    });
    document.getElementById('lab1-re').addEventListener('click', dibujar);
    if (window.Reveal && Reveal.on) {
      Reveal.on('slidechanged', function(e) {
        if (e.currentSlide && e.currentSlide.querySelector('#lab1-plot')) {
          Plotly.Plots.resize('lab1-plot');
        }
      });
    }
    dibujar();
...
**Versión interactiva (solo HTML):** dos deslizadores controlan el número de lanzamientos $n$ y la probabilidad verdadera $p$; el gráfico redibuja en vivo la frecuencia relativa acumulada de $n$ ensayos Bernoulli($p$) junto a la línea horizontal en $p$.

**Conclusión:** es la misma lógica de la simulación Monte Carlo de la lámina anterior — al crecer $n$, la frecuencia relativa $n_A/n$ converge a $P(A)$.
:::

::: {.callout-tip title="Intuición" appearance="simple"}
Cada observación nueva pesa cada vez menos en el promedio: por eso la curva se aquieta y termina pegada a la probabilidad verdadera cuando el número de lanzamientos crece.
:::
...
## Las cuatro operaciones, vistas como áreas
...
::: {.callout-tip title="Intuición" appearance="simple"}
Unión es "o", intersección es "y", complemento es "no": operar con eventos es aplicar los conectores lógicos del lenguaje a zonas del diagrama.
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Piense en un kilo de arena repartido sobre los resultados posibles: ningún montón es negativo, la arena completa suma uno, y el peso de zonas separadas se obtiene sumando sus montones.
:::
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Condicionar es hacer zoom: se descarta todo lo que quedó fuera de $B$ y lo que sobrevive se re-escala para volver a sumar uno.
:::
:::

## Condicionar es restringir el espacio muestral
...
Al saber que ocurrió $B$, los resultados fuera de $B$ dejan de ser posibles: $P(A \mid B)$ compara el área de $A \cap B$ con el área total de $B$, no con la de $\Omega$.

::: {.callout-tip title="Intuición" appearance="simple"}
El diagrama muestra el zoom: al condicionar en $B$, el rectángulo completo deja de importar y el círculo $B$ pasa a ser el nuevo universo de referencia.
:::
...
## La ley de probabilidad total en un diagrama
...
Las franjas $A_1, A_2, A_3$ parten $\Omega$; el evento $B$ (elipse) queda cortado en pedazos disjuntos y $P(B) = \sum_i P(B \cap A_i) = \sum_i P(B \mid A_i)\,P(A_i)$.

::: {.callout-tip title="Intuición" appearance="simple"}
Para medir el evento completo, recórralo pedazo a pedazo: cada franja aporta lo suyo y ninguna parte se cuenta dos veces — un promedio ponderado de las franjas.
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Bayes es la máquina de invertir condicionales: convierte "cómo se ven los datos según cada estado del mundo" en "qué estado del mundo creer después de ver los datos".
:::
:::
...
## Laboratorio: Teorema de Bayes

Mueva los controles del instrumento de focalización y observe cómo cambian el prior y los dos posteriores.

::: {.content-visible when-format="html"}

```{=html}
<div style="display:flex; gap:1.5em; align-items:center; flex-wrap:wrap; font-size:0.62em; margin-bottom:0.3em;">
  <label>Prevalencia P(V): <input type="range" id="lab2-prev" min="0.01" max="0.6" step="0.01" value="0.10" style="width:170px; vertical-align:middle;"> <span id="lab2-prev-val"></span></label>
  <label>Sensibilidad P(+|V): <input type="range" id="lab2-sens" min="0.5" max="1" step="0.01" value="0.90" style="width:170px; vertical-align:middle;"> <span id="lab2-sens-val"></span></label>
  <label>Especificidad P(-|no V): <input type="range" id="lab2-esp" min="0.5" max="1" step="0.01" value="0.85" style="width:170px; vertical-align:middle;"> <span id="lab2-esp-val"></span></label>
</div>
...
(function() {
  function dibujar() {
    var prev = parseFloat(document.getElementById('lab2-prev').value);
    var sens = parseFloat(document.getElementById('lab2-sens').value);
    var esp = parseFloat(document.getElementById('lab2-esp').value);
    document.getElementById('lab2-prev-val').textContent = prev.toFixed(2);
    document.getElementById('lab2-sens-val').textContent = sens.toFixed(2);
    document.getElementById('lab2-esp-val').textContent = esp.toFixed(2);
    var postPos = sens * prev / (sens * prev + (1 - esp) * (1 - prev));
    var postNeg = (1 - sens) * prev / ((1 - sens) * prev + esp * (1 - prev));
    Plotly.react('lab2-plot', [{
      x: ['Prior P(V)', 'P(V | +)', 'P(V | -)'],
      y: [prev, postPos, postNeg],
      type: 'bar',
      marker: {color: ['#2E86C1', '#E74C3C', '#F39C12']},
      text: [prev.toFixed(3), postPos.toFixed(3), postNeg.toFixed(3)],
    ...
    document.getElementById('lab2-lectura').textContent =
      'Posterior calculado: un "+" mueve la probabilidad de ' + prev.toFixed(2) +
      ' a P(V | +) = ' + postPos.toFixed(3) + '; un "-" la baja a P(V | -) = ' +
      postNeg.toFixed(3) + '.';
  }
  function init() {
    if (typeof Plotly === 'undefined') { setTimeout(init, 200); return; }
    ['lab2-prev', 'lab2-sens', 'lab2-esp'].forEach(function(id) {
      document.getElementById(id).addEventListener('input', dibujar);
    });
    if (window.Reveal && Reveal.on) {
      Reveal.on('slidechanged', function(e) {
        if (e.currentSlide && e.currentSlide.querySelector('#lab2-plot')) {
          Plotly.Plots.resize('lab2-plot');
        }
      });
    }
    dibujar();
...
::: {.content-visible unless-format="html"}
**Versión interactiva (solo HTML):** tres deslizadores (prevalencia, sensibilidad, especificidad) recalculan en vivo las barras de $P(V)$, $P(V \mid +)$ y $P(V \mid -)$.

Los valores iniciales reproducen el ejemplo de focalización ya resuelto: con $P(V) = 0.10$, sensibilidad $0.90$ y especificidad $0.85$ se obtiene $P(V \mid +) = 0.40$ y $P(V \mid -) \approx 0.013$. Al subir la prevalencia, el mismo instrumento comete muchos menos errores de inclusión.
:::

::: {.callout-tip title="Intuición" appearance="simple"}
El posterior mezcla lo que ya se sabía (la prior) con lo que el dato aporta: si la condición es rara, hasta un instrumento muy bueno se equivoca seguido al dar positivo.
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Dos eventos son independientes cuando enterarse de uno no cambia ninguna apuesta sobre el otro: la información no viaja entre ellos.
:::
:::
...
## En el diagrama: exclusión mutua vs independencia
...
**La exclusión mutua sí se dibuja:** círculos sin traslape. **La independencia no se dibuja:** no es una propiedad de la forma o posición de los conjuntos, sino de los **productos de probabilidades** — exige $P(A \cap B) = P(A)\,P(B)$, algo que ningún diagrama de áreas puede mostrar por sí solo.

::: {.callout-tip title="Intuición" appearance="simple"}
Excluyentes: no caben juntos — ver uno descarta al otro. Independientes: se ignoran — ver uno no dice nada del otro. Son ideas casi opuestas, no sinónimos.
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Una variable aleatoria es una etiquetadora: a cada resultado del azar le pega un número. El azar vive en el sorteo, no en la regla que etiqueta.
:::
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
La CDF responde siempre la misma pregunta: ¿cuánta probabilidad se ha acumulado hasta este punto? Por eso nunca baja y termina en uno.
:::
:::
