---
title: "Sesión 2: Fundamentos de Probabilidad"
...
En esta sesión cubriremos los fundamentos de probabilidad: conceptos básicos, reglas de probabilidad (suma, condicional), el Teorema de Bayes y su aplicación a pruebas diagnósticas, variables aleatorias discretas y continuas, y la convergencia de frecuencias relativas (Ley de los Grandes Números).
...
El **espacio muestral** es el conjunto de todos los resultados posibles de un experimento. Un **evento** es un subconjunto del espacio muestral.
...
Simulamos una encuesta con género y preferencia política para ilustrar probabilidad marginal, condicional y el test de independencia $\chi^2$.

```{r}
#| label: reglas-probabilidad
set.seed(123)
n <- 1000
...
```{r}
#| label: regla-suma
cat("--- Regla de la Suma: Baraja de cartas ---\n")
P_roja <- 26/52
P_figura <- 12/52
P_roja_y_fig <- 6/52
...
El Teorema de Bayes nos permite "invertir" probabilidades condicionales:
...
cat("P(Enfermo | +) simulada:", round(P_bayes_sim, 4), "\n")
cat("P(Enfermo | +) teórica:", round(P_enfermo_dado_pos, 4), "\n")
```

```{r}
#| label: fig-bayes
#| fig-cap: "Resultados de la simulación de prueba diagnóstica"
...
ggplot(resultados, aes(x = estado, y = conteo, fill = estado)) +
  geom_col(alpha = 0.8) +
...
## 5. Efecto de la prevalencia

```{r}
#| label: fig-prevalencia
#| fig-cap: "P(Enfermo|+) en función de la prevalencia"
...
ggplot(df_prev, aes(x = prevalencia, y = P_enfermo_pos)) +
...
Una **variable aleatoria discreta** toma valores contables. Su distribución se describe mediante la función de masa de probabilidad (PMF).
...
```{r}
#| label: fig-pmf
#| fig-cap: "Comparación entre dado justo y cargado"
df_dado <- data.frame(
  x = rep(x, 2),
...
ggplot(df_dado, aes(x = factor(x), y = prob, fill = tipo)) +
...
```{r}
#| label: fig-normal
#| fig-cap: "Normal estándar con área central al 95%"
x_norm <- seq(-4, 4, by = 0.01)
df_norm <- data.frame(x = x_norm, densidad = dnorm(x_norm))

ggplot(df_norm, aes(x = x, y = densidad)) +
...
La **Ley de los Grandes Números** establece que la frecuencia relativa converge a la probabilidad teórica conforme aumenta el número de ensayos.

```{r}
#| label: fig-convergencia
#| fig-cap: "Convergencia a P = 0.5 con más lanzamientos"
set.seed(789)
n_max <- 10000
moneda <- sample(c(0, 1), n_max, replace = TRUE)
freq_acumulada <- cumsum(moneda) / (1:n_max)

df_conv <- data.frame(n = 1:n_max, frecuencia = freq_acumulada)

ggplot(df_conv, aes(x = n, y = frecuencia)) +
...
En esta sesión revisamos: los conceptos fundamentales de probabilidad (espacio muestral, eventos), las reglas de probabilidad condicional y marginal, el Teorema de Bayes con aplicación a diagnóstico médico, variables aleatorias discretas y continuas, y la Ley de los Grandes Números. Estos conceptos son la base para la inferencia estadística que veremos en sesiones posteriores.
