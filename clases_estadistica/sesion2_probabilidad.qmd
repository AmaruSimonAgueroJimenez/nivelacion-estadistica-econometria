...
En el ejemplo de la comuna: unión $=$ recibe al menos un beneficio; intersección $=$ recibe ambos; complemento $=$ no recibe vivienda; disjuntos $=$ categorías excluyentes de un registro.

::: {.callout-tip title="Intuición" appearance="simple"}
Unión: pertenecer a al menos uno de los dos clubes. Intersección: estar inscrito en ambos a la vez. Complemento: todo el resto de la sala. Disjuntos: clubes que no comparten ningún miembro.
:::
...
::: {.callout-tip title="Intuición" appearance="simple"}
Tres maneras de responder "¿qué tan seguro estoy?": contar casos como en un juego de mesa, mirar qué pasó las mil veces anteriores, o decidir cuánto apostaría hoy y revisarlo al ver los datos.
:::
...
::: {.callout-tip title="Intuición" appearance="simple"}
Sumar los dos grupos cuenta dos veces a quienes están en ambos círculos del Venn: por eso se descuenta una vez la zona compartida.
:::
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
El azar es salvaje en lo pequeño y disciplinado en lo grande: ninguna tirada individual es predecible, pero el promedio de miles sí lo es.
:::
:::

## Laboratorio: ley de los grandes números

Mueva los controles y observe cómo la frecuencia relativa acumulada se adhiere a $p$ cuando crece $n$. Cada movimiento genera una simulación nueva.

::: {.content-visible when-format="html"}

```{=html}
<div style="display:flex; gap:1.5em; align-items:center; flex-wrap:wrap; font-size:0.62em; margin-bottom:0.3em;">
  <label>Numero de repeticiones n: <input type="range" id="lab2-n" min="100" max="10000" step="100" value="1000" style="width:170px; vertical-align:middle;"> <span id="lab2-n-val"></span></label>
  <label>Probabilidad p del evento: <input type="range" id="lab2-p" min="0.05" max="0.95" step="0.01" value="0.17" style="width:170px; vertical-align:middle;"> <span id="lab2-p-val"></span></label>
</div>
...
(function() {
  function dibujar() {
    ...
    var p = parseFloat(document.getElementById('lab2-p').value);
    document.getElementById('lab2-n-val').textContent = n;
    document.getElementById('lab2-p-val').textContent = p.toFixed(2);
    var xs = [], ys = [], s = 0;
    for (var i = 1; i <= n; i++) {
      if (Math.random() < p) { s += 1; }
      xs.push(i);
      ys.push(s / i);
    }
    ...
    var trazaP = {
      x: [1, 10000], y: [p, p], type: 'scatter', mode: 'lines',
      line: {color: '#E74C3C', width: 2, dash: 'dash'},
      name: 'p verdadera'
    };
    Plotly.react('lab2-plot', [trazaFrec, trazaP], {
      margin: {t: 10, r: 20, b: 40, l: 50},
      xaxis: {type: 'log', range: [0, 4], title: {text: 'N de repeticiones (escala log)'}},
      yaxis: {range: [0, 1], title: {text: 'Frecuencia relativa acumulada'}},
      showlegend: true,
      legend: {orientation: 'h', x: 0.5, xanchor: 'center', y: 1.08}
    }, {displayModeBar: false});
    document.getElementById('lab2-lectura').textContent =
      'Con n = ' + n + ' y p = ' + p.toFixed(2) +
      ': frecuencia final = ' + frecFinal.toFixed(3) +
      ' (desvio |frec - p| = ' + Math.abs(frecFinal - p).toFixed(3) +
      '). Con n pequeno la linea fluctua; con n grande se pega a la linea roja.';
  }
  function init() {
    if (typeof Plotly === 'undefined') { setTimeout(init, 200); return; }
    ['lab2-n', 'lab2-p'].forEach(function(id) {
...
En la versión HTML este laboratorio simula $n$ repeticiones de un evento con probabilidad $p$ (elegidos con deslizadores) y grafica la frecuencia relativa acumulada en vivo:

- Con $n < 100$ la trayectoria fluctúa con violencia: rachas largas alejan la frecuencia de $p$.
- Con $n \approx 10.000$ la trayectoria queda pegada a la línea de $p$, cualquiera sea el $p$ elegido.
- Es el mismo fenómeno del dado de la figura anterior y el fundamento de la verificación Monte Carlo del final de la sesión: **simular es calcular**.
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Marginal: mirar toda la sala. Condicional: repartir el 100% solo en la esquina donde ocurrió el evento.
:::
:::
...
## Condicionar es restringir el universo
...
$P(A \mid B)$ compara el área roja ($A \cap B$) con el área total de $B$: lo que queda fuera de $B$ deja de contar.

::: {.callout-tip title="Intuición" appearance="simple"}
Saber que $B$ ocurrió encoge el mundo: la pregunta ya no es "¿qué tan grande es A en todo el universo?" sino "¿qué fracción del territorio de B está ocupada por A?".
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Para que ocurran dos cosas en secuencia, la segunda se evalúa en el mundo donde la primera ya pasó: por eso la segunda auditoría enfrenta 3 irregulares entre 19, no 4 entre 20.
:::
:::
...
## La partición en imágenes: sumar los pedazos de A
...
Las bandas $B_1, B_2, B_3$ (las regiones) cubren todo $\Omega$ sin traslaparse; el evento $A$ (desertar) queda cortado en tres pedazos y $P(A)$ es la suma de sus áreas.

::: {.callout-tip title="Intuición" appearance="simple"}
La tasa global es una mezcla: cada región aporta su propia tasa, pesada por su tamaño. Recuperar el total es volver a juntar los pedazos del evento que quedaron en cada banda.
:::
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Bayes invierte la pregunta: de "qué tan típica es esta evidencia entre los enfermos" a "qué tan probable es la enfermedad a la vista de la evidencia". La respuesta mezcla lo que muestra el dato con lo que ya se sabía antes de verlo.
:::
:::
...
Mueva los controles del test diagnóstico y observe cómo el posterior $P(\text{Enf} \mid +)$ responde a la prevalencia, la sensibilidad y la especificidad.

::: {.content-visible when-format="html"}

```{=html}
<div style="display:flex; gap:1.5em; align-items:center; flex-wrap:wrap; font-size:0.62em; margin-bottom:0.3em;">
  <label>Prevalencia: <input type="range" id="lab1-prev" min="0.001" max="0.30" step="0.001" value="0.01" style="width:170px; vertical-align:middle;"> <span id="lab1-prev-val"></span></label>
  <label>Sensibilidad: <input type="range" id="lab1-sens" min="0.50" max="1" step="0.01" value="0.95" style="width:170px; vertical-align:middle;"> <span id="lab1-sens-val"></span></label>
  <label>Especificidad: <input type="range" id="lab1-esp" min="0.50" max="1" step="0.01" value="0.90" style="width:170px; vertical-align:middle;"> <span id="lab1-esp-val"></span></label>
</div>
...
<script>
(function() {
  function pct(x, dec) { return (100 * x).toFixed(dec) + '%'; }
  function dibujar() {
    var prev = parseFloat(document.getElementById('lab1-prev').value);
    var sens = parseFloat(document.getElementById('lab1-sens').value);
    var esp = parseFloat(document.getElementById('lab1-esp').value);
    document.getElementById('lab1-prev-val').textContent = pct(prev, 1);
    document.getElementById('lab1-sens-val').textContent = pct(sens, 0);
    document.getElementById('lab1-esp-val').textContent = pct(esp, 0);
    var pPos = sens * prev + (1 - esp) * (1 - prev);
    var post = sens * prev / pPos;
    var pNeg = (1 - sens) * prev + esp * (1 - prev);
    var postNeg = pNeg > 0 ? (1 - sens) * prev / pNeg : 0;
    var etiquetas = ['Prior: P(Enf)', 'Posterior: P(Enf | +)', 'Posterior: P(Enf | -)'];
    var valores = [prev, post, postNeg];
    var traza = {
      x: etiquetas, y: valores, type: 'bar',
      marker: {color: ['#2E86C1', '#E74C3C', '#F39C12']},
      text: valores.map(function(v) { return pct(v, 1); }),
      textposition: 'outside',
      textfont: {color: '#1F4E79', size: 13},
      width: 0.55
    };
    Plotly.react('lab1-plot', [traza], {
      margin: {t: 10, r: 20, b: 40, l: 50},
      yaxis: {range: [0, 1.05], title: {text: 'Probabilidad'}, tickformat: ',.0%'},
      xaxis: {fixedrange: true},
      showlegend: false
    }, {displayModeBar: false});
    document.getElementById('lab1-lectura').textContent =
      'P(+) = ' + pct(pPos, 1) + ' de la poblacion. De cada 1.000 positivos, ' +
      Math.round(1000 * post) + ' estan realmente enfermos; el resto son falsas alarmas. ' +
      'Un resultado negativo deja la probabilidad en ' + pct(postNeg, 2) + '.';
  }
  function init() {
    if (typeof Plotly === 'undefined') { setTimeout(init, 200); return; }
    ['lab1-prev', 'lab1-sens', 'lab1-esp'].forEach(function(id) {
      document.getElementById(id).addEventListener('input', dibujar);
    });
    dibujar();
  }
...
En la versión HTML este laboratorio permite mover prevalencia, sensibilidad y especificidad, y compara en vivo el prior con los posteriores tras un test positivo o negativo. Con sensibilidad 95% y especificidad 90%:
...
La conclusión del laboratorio: el posterior es esclavo del prior. Subir la especificidad rinde más que subir la sensibilidad cuando la prevalencia es baja.
...
::: {.content-visible when-format="html"}
::: {.callout-tip title="Intuición" appearance="simple"}
Cuando la fórmula se vuelve intratable, fabrique el mundo cien mil veces en el computador y cuente cuántas veces ocurre el evento: contar reemplaza a integrar.
:::
:::
